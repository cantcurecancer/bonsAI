...
- ★★ **Voice STT concurrent start leak (2026-07-12):** Double-tap mic before the first start RPC completed could leave an orphaned PipeWire capture session (mic stuck hot, follow-up voice broken). **Fix:** `_voice_start_generation` gate in `start_voice_transcription` / `_stop_voice_transcription_internal`. Test: `tests/test_voice_start_superseded.py`.
