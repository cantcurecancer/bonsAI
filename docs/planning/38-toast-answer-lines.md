...
**Mockup, for the two open picks (title and time on screen):** [Reply Toast Options](https://claude.ai/code/artifact/0590a00d-d48b-42ce-85be-0376c1bddf53).
Five title options drawn in the two-slot shape Decky really uses, with the time on screen and the box size to
play with, and a table of what is known from the code against what is assumed until measured. The
maintainer's other four calls are locked; see § 5.
...
## 3. Measure first, on the Deck, one block, when this is picked up

About twenty minutes, when the Deck is free and this entry is picked up. Evidence goes under `runs/` and is named in § 10.
...
4. **The title:** not yet picked. Five options are drawn in the mockup: *Reply ready*, your question, the
   game's name, *bonsAI*, and the answer starting in the title. The maintainer picks from the page.
5. **How long it stays:** not yet picked; the mockup has the slider, from two to twelve seconds. Tap to open
   is unchanged whatever the number.
...
2. **The title:** pick from mockups drawn on the real shape. Done; the page is linked at the top. The pick is
   still owed and is the only thing between this entry and a build.
3. **Time on screen:** a slider on the same page, to play with before choosing.
...
|---|---|---|---|
| 1 | A pure text helper: answer text in, preview text or nothing out. Tests: internal tags removed; every fence kind dropped, including one that would show open on screen; markdown flattened; long text cut at a word with an ellipsis; short text returned whole; only-a-fence and empty text return nothing. | Sonnet 5 high lane | the § 3 numbers and the title pick |
