...
- ★★★ **Character picker: focus ring is invisible, D-pad does not move, so you cannot pick a character.** Reported on-Deck 2026-08-04 as *"select a character, it stays on random"* plus *"the focus ring on that screen is invisible so you can't navigate via dpad"*. **These are one bug, not two, and the save path is innocent** — `useCharacterPickerModal.onOK` applies the choice locally and then writes it ([useCharacterPickerModal.tsx:64-83](../src/features/plugin-shell/useCharacterPickerModal.tsx)); it stays on random because the selection never moves off random, so OK commits what was already there. **Not caused by the step 8 extraction:** `CharacterPickerModal.tsx` was untouched by it (last real changes are `4b62886`, `7983414`, `3191b18`), and the extraction moved the opener, not the modal. **Root cause is in the modal's own focus graph:** it drives D-pad focus with `shell.querySelector(...)` lookups ([CharacterPickerModal.tsx:192-231](../src/components/CharacterPickerModal.tsx) — `focusRandomToggle`, `focusCustomCharacterField`, and friends). `.cursor/rules/decky-focus-graph.mdc` names that exact pattern as one that **misses on Deck and yields wrong spatial nav**, which is precisely the reported symptom: nothing visibly focused, D-pad inert. **Fix lean:** convert those helpers to the registered-owner pattern (`replyStopRegistry` / the new `modalReturnFocusRegistry`) instead of DOM queries. This is the same class as the entry below and they should be fixed together. **Blocks the AI-character feature entirely on a Deck** — it is only usable with a mouse today, which is why it rates higher than the picker-escape audit.
  ...
  - **Selection not sticking — fixed 2026-08-04, and it was a second, independent bug exactly as suspected.** Once the rings landed and navigation worked, picking a character still reverted to Random. Cause: `captureSessionBeforeModal` snapshots settings **before** the modal opens, and Decky's remount on modal close re-hydrates settings from that snapshot — overwriting the save the picker had just made. `patchPendingSessionSettingsSnapshot` exists for precisely this and `useOllamaModelsHubModal` already called it after its save; the character picker never did. One call added after its `save_settings` round-trip. **Original hypothesis, now confirmed:** Reported: deselect Random → pick a character → OK → Settings still shows *Random*. If the D-pad cannot move, this is downstream of the focus problem. If it also fails with a mouse, suspect the session-survival restore: on modal close `index.tsx` re-hydrates settings from `survived.settingsSnapshot`, which was captured **before** the modal opened and therefore still says `ai_character_random: true` — that would overwrite the save the picker just made. **Unverified hypothesis, worth checking first** because it would explain the symptom completely and is cheap to test with a mouse.
