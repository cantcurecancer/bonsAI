...
- ★★ **No persistent indicator of which tab you are on.** Reported 2026-08-04. Once focus is deep inside a tab — or you reopen the plugin mid-tab — the tab strip gives no sign of the active tab, so the only way to find out is to D-pad all the way up and see where focus lands. **Fix lean:** a permanent active-tab marker in the icon row (underline, filled pill, or accent tint) that does not depend on focus position. Interacts with the LB/RB icon-shuffle entry above — whatever draws the marker must not re-render the strip on every shoulder press. Check `DECKY_TAB_TITLES` and the `Tabs` wrapper in `index.tsx` for where a marker can live without fighting Decky's own tab chrome.
