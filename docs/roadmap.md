...
- ★ **Stopping an Ask in its first second keeps a raw tag fragment as the answer.** Found on-device 2026-08-04 by `scripts/probe_deck_step12_extractions.py`, which aborted a live streaming Ask ~0.5s in: the kept partial was **one character, `<`**. Streaming partials do pass through `extract_bonsai_status` and `hide_incomplete_strategy_branch_fence` ([ollama_service.py:506-507](../py_modules/backend/services/ollama_service.py)), but neither hides an **incomplete opening tag** — at that instant the accumulated text was the first character of a `<…>` tag, so the abort path's "keep the partial if it has content" test ([main.py](../main.py), `abort_background_game_ai`) saw a non-empty string and published it. The user presses Stop and the reply bubble shows `<`. **Severity: low but visible** — it only happens when Stop lands within the first tokens, and the alternative text ("Request cancelled.") already exists for the empty case. **Fix lean:** treat a partial with no alphanumeric content as empty, or hide an incomplete opening tag the way the strategy fence is hidden. **Do not** fix by lengthening the minimum partial — that would discard real short answers. Reproduce with the probe script; it is deterministic enough to hit most runs.
