...
- ★★ **Thinking blurbs — three writers disagree:** **OPEN, decided, not yet built.** The client composes an opener from a per-mount counter and the backend replaces it ~1.2s later with a different template from a per-plugin-lifetime counter, and the two languages classify intent differently (`Why does Elden Ring crash on launch?` → `troubleshooting` in TS, `generic` in Python). So the first thing the line does is change for no reason — a **THINKING-COPY-01** violation and the likely source of the "feels random" impression. **Decided 2026-08-08:** backend becomes the single writer — return the composed opener from `start_background_game_ai`, delete the ~330 lines of TS pools and classifier mirrors; fall back to *client owns the opener* only if the round-trip is visibly late on-Deck. Also queued in the same batch: emitters for `building_context` / `connecting_model` (the slow, silent stretch of an Ask), spoiler-masking the model tag inline rather than suppressing it, and accepting later `<bonsai-status>` tags so the line stops freezing for the whole generation. Fast-poll during prep is explicitly **not** being done — already reverted once for battery cost. Decisions table and rationale: [06-thinking-blurbs-review.md § 10.2](planning/06-thinking-blurbs-review.md#102-maintainer-decisions-2026-08-08).
