...
- ★★ **Session RAG preset chips never appear — RPC has no backend:** `fetchSessionRagChipCandidates` ([sessionRagChipCandidates.ts:54](../src/utils/sessionRagChipCandidates.ts)) calls `get_session_rag_chip_candidates`, which **is not implemented anywhere in Python** — the name exists only in TypeScript. With **Use local knowledge base** on, the call always rejects and the preset carousel silently falls back to static seeds, so session-scoped RAG chips have never worked. Call site: [useBonsaiAskOrchestration.ts:251](../src/hooks/useBonsaiAskOrchestration.ts). **Intent:** session-scoped chips composed from the local KB for the running game. **Fix lean:** implement the RPC on `class Plugin` returning `{ok, candidates[{text, category, prefer_ask_mode, domain}]}` — the frontend normalizer and `composePresetSeedsWithSessionRag` already exist and need no change. Decide ranking/candidate policy before building. Discovery 2026-08-02 ([phase1-map-verification.md](audit/phase1-map-verification.md)).
The session RAG gap was invisible before 2026-08-02 because the call site swallowed its rejection; it now logs to the console instead. It is not a regression — it has never worked.
...
| **D2** | **Targeted dead-code cleanup** — delete confirmed orphans; **archive** tiny-model thinking code; keep active Ask, debug, and KB-cancel paths | Proton journal RPCs + service, `apply_tdp` (+ its test-only caller), `log_navigation`, and legacy `capture_screenshot` are safe to remove after preview-suite grep. **`ask_game_ai`** stays — preview suite drives it. **`ask_ollama`** stays — every Ask calls it internally. **`dbg_fe_log`** stays — intentional on-Deck debug bridge. **`cancel_rag_corpus_download`** stays — backend cancel path exists; UI Cancel is planned (**D2 follow-up**). **`thinking_tiny_model_service.py`** is **deleted outright**; git history is the archive. Record the deleting commit SHA in this row so tiny-model thinking blurbs can be restored with one `git show <sha>:py_modules/backend/services/thinking_tiny_model_service.py`. An in-tree archive folder was rejected — the file would still surface in greps and still need explaining, which defeats the point of the cleanup. |
...
2. **D1 — wire both RPCs** — routing merge + session RAG adapter; backend/RPC tests; preview / on-Deck QA for **SESSION-RAG-CHIPS** and custom-pull routing. **D1a (routing merge) done 2026-08-02** — see Bugs § *Fixed*; on-Deck **ROUTING-MERGE-01** still open. **D1b (session RAG adapter) next.** **This is feature work, not refactor** — separate commits, not labeled behavior-preserving, and it changes what `useBonsaiAskOrchestration.ts` does at runtime. Sequenced before step 5 on purpose so the characterization tests capture intended behavior rather than the silent-fallback bug.
3. **D2 — targeted cleanup** — delete confirmed dead RPCs/modules; delete tiny-model service and record its SHA. **Preview-suite gate cleared 2026-08-02:** `tests/preview-suite/` and `scripts/` have zero hits for `proton_experiment`, `apply_tdp`, `log_navigation`, `capture_screenshot`, `dbg_fe_log`, `cancel_rag_corpus_download`, `thinking_tiny`; `ask_game_ai` has 22 hits across five tiers (keep, per D2). `apply_tdp` has no non-test reference repo-wide, so the `tier1-tdp.json` cases exercise the recommendation path only. Do not re-run this grep.
4. **Mechanical refactors** — doc one-liners; delete `refactor_helpers.py` shim; delete `settingsAndResponse.ts` barrel; split `settingsPayload.ts` (the real *and* file per audit) — one behavior-preserving commit each, tests green between. **Deploy gate on the shim commit:** `refactor_helpers.py` is referenced by `scripts/build.sh:179,208`, `scripts/build.ps1:48`, and `scripts/verify-decky-plugin-zip.sh:44`. It is the one item in the plan where every test passes and the plugin still breaks on-device, so that commit ends with a release build + zip verify + a real Deck deploy, not just `npm test`.
