...
- ★★ **You cannot ask about a game unless it is running** — **OPEN. Decision locked as [D19](audit/maintainer-decisions-locked.md#d19--can-you-reach-the-strategy-corpus-without-the-game-running); implementation deferred 2026-08-17.** Every strategy path in `should_retrieve_knowledge` ([knowledge_base_service.py:136](../py_modules/backend/services/knowledge_base_service.py)) requires `app_id` or `app_name`, so with no running game the only door into the corpus is the troubleshooting router. Asking *"how do gels work in portal 2"* from the couch attaches nothing. **Measured on device 2026-08-17** (corpus `2026.08.16`): `hl2 ravenholm` and `drg survivor what class` return `gate=False` in Speed *and* Strategy, while `ravenholm` with HL2 running retrieves the Ravenholm card normally. **Not a design choice** — `KB-NEWTITLE-01` already documents the alias path as expected behavior, so this was specified and never built. **Fix lean:** the alias table already holds `hl2` / `oot` / `drg survivor` / `portal 2`; `_resolve_game_id` (`:222`) is simply never handed the question text. Scan it as a last resort when no game is running. D19 locks spoiler fencing **on** for text-resolved titles and a **3-character** minimum alias.
...
- ★★ **Expert mode attaches fewer knowledge cards than Strategy** — **OPEN, measured on device 2026-08-17.** Two mode-keyed knobs disagree about what Expert means. `_budget_for_mode` ([knowledge_base_service.py:126](../py_modules/backend/services/knowledge_base_service.py)) gives Expert the **largest** card budget (speed 1 / strategy 3 / expert 5), but `implicit_route = (ask_mode or "").strip().lower() != "strategy"` (`:758`) puts Expert on the **strictest** relevance bar — `IMPLICIT_ROUTE_RELEVANCE_FLOOR` 4.0 versus `BM25_RELEVANCE_FLOOR` 1.0 — because the flag tests for Strategy by name rather than for "the user declared this Ask to be about the game". Measured against corpus `2026.08.16`, DRG Survivor (`2321470`), *"what class should i pick"*: **Strategy attaches 2 cards** (Classes, Upgrades and overclocks), **Expert attaches 1** (Classes — `Upgrades` scores bm25 2.13 and dies at the 4.0 floor). The mode a stuck player picks for maximum depth is the one that hides the most corpus. **Also fails the KB-ASKMODE-01 premise** that counts rise 1/3/5 with mode. **Fix lean:** Expert is as explicit a declaration as Strategy — the D17 comment at `:754` describes the distinction it wants ("without the user having declared the Ask to be about the game"), and Expert clears that bar. Widen the check to `mode not in {"strategy", "expert"}`, or key the floor off the same signal that picks the budget so the two cannot drift again.
...
- ★★★ **Compat retrieval returns a tip from the wrong topic** — **OPEN, measured 2026-08-17.** The D16 topic router works — all four `KB-ROUTER-01` sentences reach the tip sheet with hybrid engaged — but **half come back with an off-topic tip**. *"the game only responds to the touchpad and ignores the sticks"* routes to `steam_input`, then retrieval returns the **gamescope** tip *"If game ignores resolution, set in-game resolution to match gamescope target"* — a lexical match on *ignores*, beating **ten** genuinely relevant `steam_input` tips that are present in the corpus. *"my playstation 2 games run at half speed on the handheld"* routes to `emudeck` and returns a **desktop_mode** tip. **Cause:** [knowledge_base_service.py:39](../py_modules/backend/services/knowledge_base_service.py) imports only `question_targets_compat_corpus`, the yes/no gate — never `match_compat_corpus_topics`, which returns *which* topic matched. The router decides the topic, then the topic is discarded and the whole tip sheet is searched unfiltered. **Not caught by KB-EVAL-02**, which measured gate reachability (39/40), not whether the returned tip was relevant. **Fix lean:** feed the matched topic into `_search_compat_patterns` as a filter or an RRF signal.
...
- ★★★ **Hybrid retrieval's vector half adds no recall — it only re-orders what BM25 already found** — **OPEN, measured on device 2026-08-17.** Every candidate comes from one FTS query ([knowledge_base_service.py:792](../py_modules/backend/services/knowledge_base_service.py)); the vectors are then loaded **for those candidates only** — `_load_section_vectors(conn, [c.section_id for c in cards])` (`:829`) — and fused by `_fuse_cards_by_rrf` (`:830`). So RRF re-ranks a keyword shortlist; it is not a fusion of two recall paths. The guard at `:814` is `if nomic_ready and cards:`, so when BM25 returns nothing, **no embedding is computed at all** and a semantically perfect card is unreachable. Measured against corpus `2026.08.16`, DRG Survivor's 5-section title, floor 0.0 and `HYBRID_FTS_SHORTLIST_K` 30 (so neither the floor nor the shortlist is the limiter): *"how do i kill the big armoured bug boss"* → **0 candidates** with the Glyphid Dreadnought card present; *"which character is best for a beginner"* → **0** with Classes present; *"tips for the thorny plant level"* → **1, and it is the wrong card** (Upgrades and overclocks, not Hollow Bough). The literal *"how do i beat the glyphid dreadnought"* returns 1. **Two costs.** Users pay the embed round-trip for nothing on the common path — 793–900 ms measured across three modes, re-ranking a pool of **one**, where RRF is a no-op (screenshots `DeckCapture_20260817_210521` / `_210738` / `_210917`). And the **`Keyword + meaning` chip overstates what ran**: meaning never widened the search, so the label promises recall the pipeline cannot deliver. **Not caught by KB-RRF-01**, which confirmed the embed fires and timings are non-zero — never that the vector half changed which cards were found. **Fix lean:** give vectors their own recall pass (ANN or a brute-force scan over the game's sections — titles are 5–13 cards, so cost is trivial) and RRF the two lists, which is what the design in [knowledge-base.md](knowledge-base.md) describes. Phase 7's sqlite-vss item is the heavyweight version of this; a per-game scan needs no new dependency. Until it lands, either fix the chip label or scope the claim.
