...
- ★★ **Session RAG chip candidate pool is 86% filler:** For DRG Survivor (appId `2321470`) `get_session_rag_chip_candidates` returns **29 candidates, 25 of which are the same template** — `Any known <tag> issues for this game?` enumerated over compat pattern tags (`proton`, `steam_machine`, `bpm`, `desktop_mode`, `gaming_mode`, `wine`, `windows_steam`, `steamvr`, …). Only **two** name anything from the corpus: `How do I beat Glyphid Dreadnought?` and `Tips for Hollow Bough in this game?`. Measured on-device 2026-08-03 against corpus `2026.07.30`. Pool *ordering* currently rescues it — `pickRag` walks from index 0 and the two entity-named candidates sit at positions 1–2 — so the first chip a user sees is the good one. But anything past position 2 is noise, so a longer session or a wider carousel degrades fast. **This is the gap D1b deliberately left open** ("no ranking or candidate policy was designed or changed" — the RPC was wired as an adapter only). **Fix lean:** cap or collapse the per-tag compat expansion to one or two entries, and prefer entity-named candidates; do not grow the pool to fill slots. **Not in scope:** vector ranking (Phase 5).
- ★★ **Preset chips never re-roll once a session survival snapshot exists:** The carousel reseeds only on **cold mount when the snapshot has no saved prompts**, or when the running game changes ([useBonsaiAskOrchestration.ts:288-306](../src/hooks/useBonsaiAskOrchestration.ts)). `coldMountPresetReseedDoneRef` is initialized to `true` whenever `survivalPeek.suggestedPrompts` is non-empty, so **closing and reopening the plugin restores the same three chips** rather than drawing new ones. This is the actual reason a 2026-08-03 QA pass saw no RAG chips across several plugin opens: the chips were frozen from a session that predated the RAG backend, so the 30% roll never re-ran. Preserving chips across a reopen is defensible for session continuity; freezing them for the rest of the Steam session is probably not. **Decide** whether reopen should re-roll, or only a game change should. Until then the Developer-tab override below forces a reseed, which is why QA is unblocked without settling this.
