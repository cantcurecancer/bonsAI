...
- ★ **Unified input + Ask bar no longer span QAM width** — **OPEN.** Unified-input textarea and Ask bar render narrower than the QAM panel (regression window `40f396f`) — maintainer confirms this happens whether or not the AI character avatar is on, which rules out the avatar-row CSS as the sole cause and points at the shared measurement loop in `useUnifiedInputSurface.ts`. **Fix lean:** instrument before guessing again — `scripts/probe_deck_ask_row_width.py` (added 2026-08-15) dumps the full geometry/CSS-var chain with five explicit verdicts (V1–V5); run it on-Deck first, then fix the one landing zone it indicts. Files: [MainTabUnifiedAskBar.tsx](../src/components/MainTabUnifiedAskBar.tsx), [section-5.ts](../src/styles/sections/section-5.ts), [useUnifiedInputSurface.ts](../src/features/unified-input/useUnifiedInputSurface.ts), [probe_deck_ask_row_width.py](../scripts/probe_deck_ask_row_width.py).
