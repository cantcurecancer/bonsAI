...
- ★ **Session RAG chips are hard to QA — 30% per-slot probability:** `SESSION_RAG_CHIP_PROBABILITY = 0.3` in [sessionRagComposer.ts](../src/features/preset-carousel/sessionRagComposer.ts), rolled independently per static-seed slot, so a carousel showing a handful of chips often displays **zero** RAG chips even when the backend is working correctly. That is fine as product behavior and bad as a test procedure — it cost a QA cycle on 2026-08-03, where the tester reasonably concluded the feature was not working. **Fix lean:** a Developer-tab toggle to force probability to 1.0 for a session, which makes **SESSION-RAG-CHIPS-01** deterministic. Until then the row documents a repeat-until-seen procedure.
