...
- ★★★ **Spoiler "tap to reveal" cannot be focused with a D-pad — STILL BROKEN after one attempt 2026-08-04.** The diversion described below shipped and **did not work on-device**. **Most likely reason, and the same trap as the character picker:** the diversion calls `el.focus()` on the fence, but a Decky `Focusable` is navigated through Decky's own virtual-focus model — a raw DOM `.focus()` does not update it, so nav state never moves and the next press is routed as if nothing happened. `.cursor/rules/decky-focus-graph.mdc` warns about exactly this ("register mounted focus owners and **return true** from `onMove*`"), and the working examples in this repo move focus by returning `true` from a parent handler, not by calling `.focus()` on a nested `Focusable`. The maintainer also observed what looks like a container wrapping the spoiler area, consistent with the bubble's `Focusable` preempting child navigation entirely. **Next step is instrumentation, not a third attempt** — log whether the diversion fires and whether Decky's focus actually moves. **Do not simply retry with different timing.** Attempted fix: Cause: the reply bubble is a **single** `Focusable` whose chunks are plain divs ([buildAnswerBubbleElement.tsx:111](../src/utils/buildAnswerBubbleElement.tsx)), so the fence's own `Focusable` never received focus — D-pad Down scrolled straight past it. Fixed by diverting in `handleAnswerBubbleMoveDown`: before scrolling, if a still-masked fence is on screen, focus it. **Deliberately a diversion rather than a restructure** — making the chunks their own focus stops would mean rewriting the bubble's scroll-step logic, which the choppy-scroll entry below says not to disturb without on-Deck proof. Fences are tracked in a registry (`spoilerFenceRegistry`) rather than found by selector, per the focus rules, and each is offered **once**: A reveals, Down again scrolls on, so a spoiler you skip cannot trap you. Spoiler fences are `Focusable` divs rather than buttons, so they also needed their own ring rule. **11 tests.** Original report: The reveal control is touch-only, so on a Deck used from the couch — or on any device without a working touchscreen — **there is no way to uncover masked strategy text at all**. Not a polish item: it makes the spoiler feature a dead end for the primary input method, and the masking is on by default. **Fix lean:** make the reveal a real focus stop in the reply's focus graph (registered owner, per `.cursor/rules/decky-focus-graph.mdc`), reachable from the answer body, with A to reveal. Needs a D-pad row in [testing-manual.md](testing-manual.md) alongside the existing spoiler rows.
