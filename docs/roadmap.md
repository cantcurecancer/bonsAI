...
- ★ **British spelling finds nothing** — **OPEN, found 2026-08-18.** Searching `armor` returns the Glyphid Dreadnought card; searching `armour` returns **nothing**. Same for `armored` / `armoured`. FTS5's porter stemmer normalises word endings, not spelling variants, and every card in the corpus is written in US English, so a player who types the way they speak misses cards that are sitting right there. Measured against corpus `2026.08.16`: *"how do i kill the big armoured bug boss"* → 0 keyword candidates, the identical question with `armored` → the right card, first hit. The meaning search added 2026-08-18 covers for it (the question now attaches the card either way) but only because the sentence has other signal — `armour` **alone** still returns nothing. Affects the obvious set: armour, colour, defence, behaviour, customise, licence, grey. **Fix lean:** normalise the query in `_expand_query`, not the corpus — a short British→US map costs no rebuild and cannot desync from the cards, whereas editing card text would have to hold for every card ever added. Add the pair to the eval fixture at the same time or nothing guards it.
- ★ **You cannot ask for "the boss" — a card's type is not searchable** — **OPEN, found 2026-08-18.** Every section carries a type (`boss`, `area`, `mechanic`) and it is shown to the model in the attached block, but `sections_fts` indexes **`(name, card)` only** ([knowledge_base_schema.py:90](../py_modules/backend/services/knowledge_base_schema.py)), so the type is invisible to search. Measured on corpus `2026.08.16`, DRG Survivor: *"how do i beat the boss"* → **0 keyword candidates**, and the meaning search does not rescue it either — it attaches Mining, Upgrades and Hollow Bough, with the one card actually marked `boss` **absent**. `area` and `mechanic` behave the same. A player who does not know the boss's name — which is the player most in need of the card — cannot reach it. **Fix lean needs a call:** indexing the type makes `boss` match every boss card at once, which is right for *"how do i beat the boss"* on a single-boss title and noisy on a title with twelve. Options are (a) add `section_type` to the FTS table (schema change + corpus rebuild), (b) expand the query instead — map a bare *"the boss"* to the game's boss-type sections at retrieval time, no rebuild, or (c) treat type as a ranking boost rather than a match. Related: the same absence is why *"who is the little robot"* attaches only the genre fallback.
