...
- ★★ **Sweep the remaining global-`document` lookups — same root cause as the spoiler bug, 8 files.** [docs/audit/decky-realms.md](audit/decky-realms.md) lists them with line numbers: `chatPanelScroll.ts:80`, `focusNavigation.ts:71`, `settingsPanelScroll.ts:21`, `MainTabChatTranscript.tsx:203/214/215`, `MainTabUnifiedAskBar.tsx:737`, `MainTabPresetAnimatedChips.tsx:157`, `AboutTab.tsx:99`, `useBonsaiAskOrchestration.ts:730`. Each is a focus or scroll helper that silently does nothing on device. The fix per site is mechanical — a ref, an element-scoped query, or `getUiDocument()` — but each one is a behaviour change on a path with no test, so they want on-Deck confirmation in small batches rather than one sweep. **Also in this family, and worth doing first:** `useBonsaiAskOrchestration.ts:730` uses `instanceof HTMLElement`, which is **false** for a node from the other realm — brand checks never work across realms, so that blur never fires. **Related:** the character-picker focus graph below drives D-pad focus with `shell.querySelector(...)`; confirm which document `showModal` portals into before assuming this fix reaches it.
- ~~★★ **No persistent indicator of which tab you are on**~~ — **fixed 2026-08-04, awaiting on-Deck confirmation.** Reported 2026-08-04: once focus is deep inside a tab — or you reopen the plugin mid-tab — the tab strip gives no sign of the active tab, so the only way to find out is to D-pad all the way up and see where focus lands. **There was already an active-tab style; it was simply too faint to see.** [section-1.ts:166-172](../src/styles/sections/section-1.ts) drew the active chip as a **1px ring at 20% alpha** plus a 6px glow at 12% whenever focus was not on the strip, and the active glyph went white against `rgba(168, 182, 198, 0.62)` for the rest. On a Deck screen that reads as nothing. `.Active` itself was never in doubt — the comment at [:208-209](../src/styles/sections/section-1.ts) records measured on-device nesting around that exact class. **Fix, maintainer's pick of three mockups: accent icon tint.** The active glyph now takes the character accent (`--bonsai-ui-tab-active-icon`, new in [characterUiAccent.ts:148](../src/data/characterUiAccent.ts)), keyed on `.Active` with **no focus qualifier**, so it survives focus in the body and a reopen mid-tab. Result is a three-state strip: grey = another tab, white = the non-active tab the strip is pointing at, accent = the tab you are on. **Colour only, deliberately** — a filled pill or underline was mocked up and rejected; drawing no box means leaf geometry cannot change, so the marker cannot push the strip out of the 48-56px stable window `useTabStripBodyOffset` measures ([useTabStripBodyOffset.ts:44-46](../src/hooks/useTabStripBodyOffset.ts)), and CSS-only means it satisfies "must not re-render the strip on every shoulder press" absolutely. The rule's specificity walks the real `leaf > shell > icon` nesting to outrank the two white-icon rules above it, whichever one Decky's class nesting matches; the white one that was in the active-unfocused rule is gone so the active glyph has a single source of truth. Accent-following matches the strip rings and is the opposite of the gamepad focus rings, which are white literals on purpose. Regression row **TAB-MARKER-01** in [testing-manual.md](testing-manual.md). **1 test** (the marker tracks the character main, and is absent so the green literal applies when no character is set).
...
- ★ **Developer toggle for "resume last tab" (D15 B).** Requested 2026-08-04. Add a Developer-tab switch controlling the tab-resume behavior shipped as D15 option B: **on** = current behavior (reopen where you left off); **off** = reopening from any tab other than Main returns you to **Main**. Gives option A back without another code change, and makes the two behaviors comparable on-device. Default should stay **on**, matching the locked decision. Needs a settings key, the usual six-file settings path, and a focus-graph entry for the new control.
