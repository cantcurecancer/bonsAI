...
- ★★ **Named chat slots silently drop every turn (data loss)** — **FIXED 2026-08-16, on-Deck verification owed.** **Root cause: `ensureActiveSlotForAsk` was written, exported, unit-tested — and never called by any production code.** It exists in [useChatSlots.ts:143](../src/hooks/useChatSlots.ts) precisely to create a slot when none is active and hand back the id, and the only references to it in the tree were its own definition, its return object, and `useChatSlots.test.ts`. So on a fresh session `activeSlotIdRef.current` stayed `null` (it initialises from session survival, which is null on a cold start), the payload spread `...(a.activeSlotIdRef?.current ? …)` contributed nothing, and the backend dropped both turns. **Fix:** the Ask submit path now resolves a slot id *before* the RPC — `useBonsaiAskOrchestration` takes an `ensureActiveSlotForAsk` arg and sends what it returns. Free on the common path (the function returns the active id without a round trip when one is selected) and best-effort by design: if slot creation fails the Ask still runs, unpersisted, exactly as before. Wired through `ensureActiveSlotForAskRef` in [index.tsx](../src/index.tsx), matching the `reloadSlotTranscriptRef` indirection that exists because `useChatSlots` is declared after the orchestration hook. Two tests added; a mutation reverting the payload to the ref turns the first one red. **Why no gate caught it:** the unit tests covered the function and the RPC contract on both sides — nothing asserted that anything *called* it. Original report follows. — Found on-Deck 2026-08-16 during KB QA, from the plugin log — no slots test was running, and nothing on screen looks wrong: the reply renders normally and only the *persistence* is lost. Every Ask logs `chat_slots: no slot for request_id=N — assistant turn dropped` (both Asks that session, after successful 23s replies), and the v2 store `chat_slots/` ([chat_slot_service.py:22](../py_modules/backend/services/chat_slot_service.py)) **has never been created** on the QA Deck — the settings dir holds only the pre-v2 `chat_threads/`, last written 2026-07-17, months before v2 landed. **Mechanism:** the slot id is parsed out of the question text — `chat_slot_id = Plugin._parse_chat_slot_id(question)` at [main.py:2655](../main.py) — and *both* the user turn and the `_chat_slot_by_request` mapping are written only inside `if chat_slot_id:`. With no id, the user turn is dropped **silently** and the assistant turn **loudly** at [main.py:2465](../main.py), which is why the log shows half the damage and the UI shows none. **Fix lean:** start on the frontend — establish whether the Ask path sends a slot id at all, and whether its form is what `_parse_chat_slot_id` expects; a question-text marker is a fragile contract for something with no compile-time check across the TS↔Python boundary. **Blocks** all six **CHAT-SLOTS-V2-01…06** rows, and a reopen test would read as "empty thread" without pointing at the cause. Detail: [testing.md](testing.md) CHAT-SLOTS-V2 row.
...
- ★★★★★ **Named chat slots** (labeled threads — redesign only)
  ...
  - **Blocked by a bug found 2026-08-16:** the feature persists nothing on device — every turn is dropped before it reaches `chat_slots/`. All six QA rows are unrunnable until it is fixed. Entry: [Bugs](#bugs) § *Named chat slots silently drop every turn*.
