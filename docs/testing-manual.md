...
- [ ] **CHAT-SLOTS-V2-02** `[+]` creates a slot; **A** on title opens rename; **Right** → **×** → ConfirmModal deletes; focus returns to row after each modal  — **PARTIAL 2026-08-30 (automated).** `[+]` created a slot; **A** on the title opened the rename modal; **Right** moved the inner stop to **×** and turned it red without shifting the row. **Focus does not return to the row after the modal closes** — it lands on the bonsAI tab strip (PICKER-FOCUS-01). Actually deleting a slot was not exercised.
