...
- [ ] **ASK-WIDTH-01** Main tab, AI character **off** — the input box and the Ask bar reach the same left and right edges as the tab content above them (no inset from the QAM panel edges); repeat with the character **on**; repeat after LB/RB away and back. Run `scripts/probe_deck_ask_row_width.py` first if either fails — it prints which of V1–V5 is the cause instead of guessing
