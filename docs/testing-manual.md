...
- [ ] **CHAT-SLOTS-V3-05a** (W15) Ask in slot A, **LB** to slot B mid-stream: B shows B's own content (or the empty-slot preview) and zero of A's tokens; the ask bar still reads busy — FAIL (Deck) 2026-09-04: slot B showed a Strategy branch block belonging to A ("Where are you at in … ? A. Just starting in the town area / B. Dealing with a tough encounter or trap"), still there after A finished; B's ask bar did not read busy. Filed under Bugs.
