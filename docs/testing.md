...
| TDP suggestions (read-only; no sysfs apply) | SMOKE-B | Open | Apply path removed 2026-07-30; re-verify suggestion-only banner/behavior. 2026-08-02: the sysfs **write** code is gone too (`apply_tdp`, `write_sysfs`, `append_sandbox_sysfs_write`) — reads and clamp bounds are unchanged. `tests/test_tdp_sandbox_sysfs.py` is replaced by `tests/test_tdp_service.py`, and the `UNIT-B-pytest-sandbox-tdp` preview gate now runs that file instead. `get_input_transparency` still reports `sysfs_writes`, which is now always empty |
