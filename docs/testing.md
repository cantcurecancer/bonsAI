...
| KB remove (no spurious timeout / stale toggle) | KB-REMOVE-01 | Open | 2026-08-02 fix: `remove_rag_corpus` uses raw `call()` (multi-GB rmtree), pauses debounced save, then `syncSettingsFromDisk` — confirm Remove succeeds on large corpus and **Use local knowledge base** stays off after remove |
| VAC check Ask start (slow Steam API) | VAC-TIMEOUT-01 | Open | 2026-08-02 fix: `start_background_game_ai` uses 30s deadline (Steam GetPlayerBans HTTP timeout is 18s) — run `bonsai:vac-check <steamid>` on-Deck with Web API enabled; must not false-fail at 15s |
