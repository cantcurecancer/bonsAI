...
| Session RAG preset chips | SESSION-RAG-CHIPS-01 | Partial | **Backend verified on-device 2026-08-03**: with the real settings and corpus `2026.07.30`, `get_session_rag_chip_candidates("2321470", …)` returns `ok: true` and 29 candidates, the first two being `How do I beat Glyphid Dreadnought?` and `Tips for Hollow Bough in this game?` — drawn from the corpus's DRG Survivor sections (`boss` / `area`). KB on, corpus at `/home/deck/.bonsai/rag`. **UI sighting still unconfirmed.**<br><br>**How to tell a RAG chip from a static seed** — this is the trap that cost a QA cycle. Generic wording like *"Help me beat this boss or encounter"* is a **hardcoded static seed** ([presets.ts:71](../src/data/presets.ts)), not a RAG chip. A real RAG chip **names the entity**: look for the literal strings **`Glyphid Dreadnought`** or **`Hollow Bough`** for DRG Survivor.<br><br>**Procedure:** KB on, corpus installed, launch DRG Survivor (appId `2321470`), open the QAM plugin and let the carousel cycle. Each seed slot has only a **30%** chance of being swapped for a RAG chip (`SESSION_RAG_CHIP_PROBABILITY`), so **expect to reopen the plugin several times** before one appears — seeing none on a single open proves nothing. Then: strategy chips must open in **Strategy** mode; turn KB **off** and confirm a return to static seeds with no console error; uninstall the corpus and confirm the same (reason `corpus_missing`, no rejection). Two quality issues found while verifying this are filed under [Bugs](roadmap.md#bugs) |
