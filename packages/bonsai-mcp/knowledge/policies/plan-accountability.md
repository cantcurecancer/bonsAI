...
- That section must list: **which agents** (by name), **whether their output was reviewed**, a **short summary of actionable findings** (or `No issues found`, or `N/A — scope did not apply` with one-line reason), and **where findings are recorded** — use `bonsai.report.archive` MCP tool and/or update `docs/archive/reports/*-report.md` when those files are the canonical published snapshot (e.g. `docs/archive/reports/security-audit-report.md`, `docs/archive/reports/foss-advocate-report.md`).
