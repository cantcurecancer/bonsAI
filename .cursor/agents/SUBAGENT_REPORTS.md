...
- **Triage:** triaged
- **Findings:** Screenshot skill inflated (Shell list gitignored screenshots/, keyword description). screenshot-ingest + deck-dev-loop + master-debugger wired. Focus bugs: main-agent checklist first, Task master-debugger only for real debug loop. sessionStart hook + bonsai.session.bootstrap now emit policy ids+fetch hints via formatSessionBootstrap (not full ALWAYS/NEVER dumps). screenshots/README.md tracked via gitignore exception.